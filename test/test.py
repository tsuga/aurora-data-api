...
class TestAuroraDataAPI(AsyncTestCase):
    ...
    async def _async_setUpClass(cls):
        ...
        async with await aurora_data_api.connect(database=cls.db_name, aurora_cluster_arn=cls.cluster_arn, secret_arn=cls.secret_arn) as conn:
    ...
    async def test_invalid_statements(self):
        async with await aurora_data_api.connect(database=self.db_name, aurora_cluster_arn=self.cluster_arn, secret_arn=self.secret_arn) as conn:
            cur = await conn.cursor()
    ...
    async def test_continue_after_timeout(self):
        ...
        try:
            ...
            async with await aurora_data_api.connect(
